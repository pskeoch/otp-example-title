	...
	OTP.Gallery = function(){
		...
		this.prevGroupClicked = function(event){
			...
			if(group_pos < 0){ // if thumb group is not in first position (not showing first group of 10)
				...
				if(group_pos >=0){
					...
					$(".btn_gallery_prev_group").attr("src","./assets/apps/gallery/btn_prev_group_off.gif").unbind("click");
				}
				group_pos = group_pos + "px";
				// Slide thumb group to new position
				var tl = new TimelineLite({onComplete:function(){ $(".btn_gallery_prev_group").unbind("click").on("click", otpGallery.prevGroupClicked );}});
				tl.to($(".gallery_thumb_group"), 1.0, {css:{"left":group_pos},ease:Power3.easeOut}, 0);
				$(".btn_gallery_next_group").attr("src","./assets/apps/gallery/btn_next_group.gif").unbind("click").on("click",otpGallery.nextGroupClicked );
			}else{
				console.log("gallery_thumb_group is at beginning");
				$(".btn_gallery_prev_group").unbind("click").on("click", otpGallery.prevGroupClicked );
			}
		}
		this.nextGroupClicked = function(event){
			...
			if( new_pos -370 <= end_pos ){
				$(".btn_gallery_next_group").attr("src","./assets/apps/gallery/btn_next_group_off.gif").unbind("click");
			}else{
				$(".btn_gallery_next_group").attr("src","./assets/apps/gallery/btn_next_group.gif").unbind("click").on("click",otpGallery.nextGroupClicked );
			}
			...
			var tl = new TimelineLite();
			tl.to($(".gallery_thumb_group"), 1.0, {css:{"left":group_pos},ease:Power3.easeOut}, 0);
			$(".btn_gallery_prev_group").attr("src","./assets/apps/gallery/btn_prev_group.gif").unbind("click").on("click",otpGallery.prevGroupClicked);
		}
		...
		$(".content").fadeOut( "fast", function() {
			...
			if( typeof pics["gallery"]["album"]["img"].length === 'undefined' ){
				...
				<img class="btn_gallery_next_group" src="assets/apps/gallery/btn_next_group.gif" width="13" height="10" style="margin-left:0px;"/>\
			...
			$(".btn_gallery_next_group").on("click",otpGallery.nextGroupClicked );
...
OTP.Intro = function () {
	...
	*/
	...
	this.onTermsSelect = function(id){
		...
		$("body").prepend('<div id="terms_copy_popup"><span class="close_btn"><span>X</span></span>'+ languages[id]["#cdata-section"] +'</div>');
...
// old animation block here
	...
	*/
	...
	for(var item in languages){
		...
		terms_btns_htm += "<div id='terms_copy"+item+"' class='terms_copy'>"+languages[item]['#cdata-section']+"</div>";
