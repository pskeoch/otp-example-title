...
OTP.introExit = function(e){
	...
	$( "#bg_gallery_pane" ).css({"display":"block"})
	otpMainMenu.create();
	//setTimeout(function() { otpContentPane.create(); }, 1000);
	//setTimeout(function() { otpBackground.create(); }, 1000);
	otpContentPane.create();
	//Only create gallery pane and background image on screens larger than 400px
	if (screenw>400) {
		otpBackground.create();
	}
	introdone = true;
	screenResize(screenh,screenw);
}
// ================================================================
OTP.BackgroundGallery = function(){
	...
	this.create = function(){
		...
		$("#bg_gallery").append('<div class="bg_top"><img src="assets/pics/bg_1.jpg "/></div>');
		// Add color styles
		aspectratio = 500/750;

//$(".bg_gallery_pane_bg").css({'background-color':OTP.colors_ar['galleryPaneColorValue'],"width":$("#interface_area").width()+"px","height":$("#interface_area").height()+"px"});
	...
	this.updateBackgroundImage = function(img){
		...
		aspectratio = 500/750;
		$(".bg_bottom img").css({"width":$('#interface_area').width() + "px","height": $('#interface_area').width()*aspectratio + "px"});
		});
		aspectratio = 500/750;
		$(".bg_top img").css({"width":$('#interface_area').width() + "px","height": $('#interface_area').width()*aspectratio + "px"});
...
OTP.ContentPane = function(){
	...
	this.displayPane = function(){
		...
		if(otpContentPane.pane_state != "open"){
			...
			tl.to($("#content_pane"), 0.5, {css:{"bottom":"640px"},ease:Power3.easeOut}, 0.0);
		}
	...
	this.showHeights = function(){
		...
		if(typeof $(".content .btn_group")[0] != "undefined"){
			...
			total_content_h = total_content_h + h;
			$(".btn_group .pane_btn").width("270px");
		}
		...
		if(total_content_h > $(".content").height()){
			...
			$(".section_btn_group .pane_btn").width("270px");
			$(".section_link_group .pane_btn").width("270px");
			$(".btn_group .pane_btn").width("270px");
			$(".column1 .pane_btn").width("270px");
			$(".column2 .pane_btn").width("270px");
		}
	}
	...
	*/
	OTP.Gallery = function(){
		...
		this.completeGalleryUpdate = function(){
			...
			$('.gallery_top img').width($('.gallery').width()-30 + "px");
			$('.gallery_bottom img').width($('.gallery').width()-30 + "px");
		}
		this.updateGalleryImage = function(img){
			...
			$(".gallery_image").prepend('<div class="gallery_bottom"><img src=""/></div>');
			$('.gallery_top img').width($('.gallery').width()-30 + "px");
			$('.gallery_bottom img').width($('.gallery').width()-30 + "px");
		...
		// fadeout complete, build the new content
			...
			//If on phone portrait mode gallery does not display well, so just display message saying it needs to be viewed on a different sized screen
			//if ($(window).width()>400) {
			
		
				$(".content").prepend('<div class="gallery"></div>');
					
				//Define gallery size based on content area size
				//Gallery image sizes typical 476x298px
				gallimgaspect = 298/476;
				if ($(".content").width()>600) {
					$('.gallery').css({"width": "496px"});
					$('.gallery').css({"height": $('.gallery').width() * gallimgaspect + 80 + "px"});
				} else {
					$('.gallery').css({"width": 496 -(600-$(".content").width()) + "px"});
					$('.gallery').css({"height": $('.gallery').width() * gallimgaspect + 80 + "px"});
				}
				...
				if( typeof pics["gallery"]["album"]["img"].length === 'undefined' ){
					...
					$(".gallery").append('<div class="gallery_image"><div class="gallery_top"><img src="./'+lgPath+p[0]["src"]+'"/></div></div>');
					$('.gallery_top img').width($('.gallery').width()-30 + "px");
					...
					$('.gallery_nav').css({"top": $('.gallery_top img').width()*gallimgaspect+20 + "px"});
			...
			//} else { //else if screen width less/equal to 400
				//$(".content").prepend('<p>Picture gallery not available in this screen size</p>');
			//}
	...
	this.showPaneW = function(root_pane){
		...
		$(".content").fadeOut( "fast", function() {
			...
			if(typeof wallpaper_list_obj !== 'undefined'){
				...
				//$(".wallpaper_list").append('<div class="column1" style="margin:0; width:49%;"></div>');
				//$(".wallpaper_list").append('<div class="column2" style="margin:0; width:50%;"></div>');
					...
					*/
					...
					for(i=0; i < wallpaper_list_obj["wallpaper"].length; i++){
						...
						if(wallpaper_list_obj["wallpaper"][i]["thumb"] != "undefined"){
							wp_group = ' <div class="wallpaper_group"><img style="display:inline;float:left;" src="'+wallpaper_list_obj["wallpaper"][i]["thumb"]+'"/> ';
								...
								wp_group += ' </div></div>';
							if(i%2===0){
								$(".wallpaper_list").append(wp_group);
							}else{
								$(".wallpaper_list").append(wp_group);
							}
						}
	...
	this.create = function(){
		...
		//$("#main_menu").after('<div id="content_pane"></div>');
		//$("#content_pane").css({"width":"500px","height":"1040px"});
		//$("#content_pane").prepend('<div class="content_tab"><div class="content_tab_bg"></div><img src="assets/pics/tab_arrow_up.png"/></div>\
		$("#content_pane").prepend('<div class="content_bg"></div>');
		//Trying content pane without background color - background color over all interface area instead to cover whole background gallery image
		...
		$(".content_bg").css({'background': 'none',"width":$("#content_pane").width()+"px","height":$("#content_pane").height()+"px"});
...
OTP.MainMenu = function(){
	...
	this.create = function(){
		...
		//Now in HTML code itself
		// Set menu position - now done through css and screenresize javascript
		...
		$("#main_menu").prepend('<img class="nav-top-img" src="assets/pics/leftNav_top.png" width="240px" align="center" height="70"/><br/>');
		$("#main_menu").append('<img class="nav-bottom-img" src="assets/pics/leftNav_bottom.png" width="240px" height="70"/><br/>');
		...
		for(i=0;i < OTP.title_data["BrandInterface"]["main_nav"]["nav"].length; i++){
			...
			$("#menu_list").append( '<li><a class="mainNavBtn_'+i+'" href="'+i+'"><img class="leftNav_arrow_left" src="assets/pics/leftNav_arrow_left.png" width="7" height="7"/>' + menu_item +' <img  class="leftNav_arrow_right" src="assets/pics/leftNav_arrow_right.png" width="5" height="8"/></a></li>' );
			if(i == 0){
			...
			$(".mainNavBtn_"+i).on("click", function(event){
				...
				$('html, body').animate({
					scrollTop: $("#content_pane").offset().top
				}, 1000);
				return false;
			})
		}
		
		$('.menu_bg').css({"width": $("#main_menu ").width() + "px","height": $("#main_menu").height() + "px"});
		logoaspect = 70/240;
		$('#main_menu img.nav-top-img').css({"width":$("#main_menu").width() + "px","height":$("#main_menu").width()*logoaspect + "px"});
		// Show/Hide graphic left arrow on button depending on:
		// Rollover or active, show the left arrow
		//$("#").css();
		
		// Slide constructed menu into view - SWITCHED OFF FOR NOW
...
OTP.Intro = function () {
	...
	*/
	...
	this.onLanguageSelect = function(id){
		...
		*/
		tl.to($('#language_menu'), 0.5, {css:{top: langmenudrop-60 + "px"},ease:Strong.easeOut}, 0)
			...
			.to($('#language_menu'), 0.5, {css:{top: langmenudrop + "px"},ease:Strong.easeOut}, 0);
	...
	this.onTermsSelect = function(id){
		...
		$("body").prepend('<div id="terms_copy_popup" style="width:' + $(window).width()*0.8 + 'px"><span class="close_btn"><span>X</span></span>'+ terms_copy +'</div>');
		$("#terms_copy_popup").css({"width":$(window).width()*0.8 + "px"});
	...
	this.onContinueSelect = function(){
		...
		*/
		tl.to($('#language_menu'), 0.9, {css:{top:langmenudrop-60 + "px"},ease:Strong.easeInOut}, 0)
			...
			.to($('#intro_area'), 1, {css:{marginLeft:$(window).width()},ease:Power2.easeInOut}, 0)
	...
	this.introAnimation = function(){
		...
		//Get screen size to determine scale of intro graphics
		if ($(window).width()>=1000) {
			var intrografwid = 1000;
			var intrografhght = intrografwid * 0.2666;
		} else {
			var intrografwid = $(window).width();
			var intrografhght = $(window).width() * 0.2666;
		}
		//Also determine offset of intro graphics from top
		langmenuhght = 50; //rough assumption of height
		totintrodisphght = intrografhght+langmenuhght;
		halfscrnhght = $(window).height() / 2;
		var intrograftop = halfscrnhght - (totintrodisphght/2);
		$("#intro_area").css({"top": intrograftop + "px"});
		
		//Determine option menu drop position
		var langmenudrop = intrografhght + $('#intro_area').offset().top;
		...
		var newimg = $("#intro_content img").attr('src', './assets/pics/intro.png' ).load(function() {
			if (!this.complete || typeof this.naturalWidth == "undefined" || this.naturalWidth == 0) {
				...
				var tl = new TimelineLite();
				introleftoffset = ($(window).width()*0.5)-(intrografwid*0.5);
				tl.to($("#intro_content img"), 0, {css:{"opacity":"1"},ease:Power3.easeOut}, 0);
					...
					*/
					tl.to($("#interface_area"), 0.8, {css:{opacity:"1"},ease:Strong.easeInOut}, 0.2)
						.to($('#intro_area'), 1.0, {css:{marginLeft: introleftoffset + "px"},ease:Power2.easeInOut}, 0)
						.to($('#intro_content'), 0.1, {css:{opacity:"1"},ease:Strong.easeOut}, 0)
						.to($('#intro_area'), 1, {css:{height:intrografhght + "px",marginTop:'0px'},ease:Strong.easeOut}, 0)
						.to($('#language_menu'), 0.5, {css:{top:langmenudrop + "px",opacity:"1"},ease:Strong.easeOut}, 0);
					
			}
		});
		
		$('#intro_area').css({"marginLeft":intrografwid*0.9 * -1,"marginTop":"-1px","height":"3px"});
		$('#intro_area').css({"width":intrografwid + "px"});
		introleftoffset = ($(window).width()*0.5)-(intrografwid*0.5);
		$('#language_menu').css({"width":intrografwid + "px","marginLeft":introleftoffset + "px"});
		$('#intro_content img').css({"height":intrografhght + "px","width":intrografwid + "px"});
...
// old animation block here
	...
	*/
	...
	$('html, body').animate({
		scrollTop: $("#interface_area").offset().top
	}, 1000);
...
//OTP.screenResize();
otpBackground.setPaneState("closed");
var introdone = false;